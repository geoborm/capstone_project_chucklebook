...
def login():
    ...
    if form.validate_on_submit():
        ...
        if comic is not None and comic.check_password(password):
            login_user(comic)
            flash(f'HAHA! You have succesfully logged in as {email}!', 'success')
            return redirect(url_for('index'))
        else: 
...
def edit_profile(profile_id):
    ...
    form.twitter_url.data = profile_to_edit.twitter_url
    form.instagram_url.data = profile_to_edit.instagram_url
    form.facebook_url.data = profile_to_edit.facebook_url 
    form.tiktok_url.data = profile_to_edit.tiktok_url 
    form.youtube_url.data = profile_to_edit.youtube_url 
    return render_template('edit_profile.html', form=form, profile=profile_to_edit)

@app.route('/delete/<int:profile_id>')
@login_required
def delete_post(profile_id):
    profile_to_delete = Com_Profile.query.get_or_404(profile_id)
    if profile_to_delete.comic != current_user:
        flash("You do not have permission to delete this profile", "danger")
        return redirect(url_for('index'))

    db.session.delete(profile_to_delete)
    db.session.commit()
    flash(f"{profile_to_delete.title} has been deleted", "info")
    return redirect(url_for('index'))